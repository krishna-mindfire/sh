...
let loopForVideoFunction = async () => {
    console.log('loopvideo function');
    ...
    (async function loop() {
        if (video_image_div.style.display != "none") {
            ...
            if (!result) {
                ...
                if (checkAudioIsPlaying()) {
                    audioEnsureProperLighting.play()
                }
...
function calculateImagePdTrigger()
{
    loaderImage.style.display = "flex";
    calculateImagePd.style.display = "none";
    calculateImagePdManually.style.display = "none";
...
function calculate_distance_trigger()
{
    loader.style.display = "flex";
    calculate.style.display = "none";
...
var calculateManuallyValueFunction = () => {
    ...
    addForm(segHeightInProgressiveMM, segHeightInBiFocalMM);
}
...
const roundToNearest5 = (num) => Math.round(num / 50) * 50;
...
function addForm(shmm, shbmm)
{
    stopVideo();
    let formHTML = '<form id="sd" action="/browsersh/storeSH" method="post" class="row g-3 pd-form">' +
            '<div class="row g-3">' +

            '<div class="col-md-4">' +
            '<label class="form-label" for="email">Name</label>' +
            '<input type="text" name="name" class="col-md-4 form-control" id="id_name">' +
            '</div>' +

            '<div class="col-md-4">' +
            '  <label class="form-label" for="email">Actual Bifocal</label>' +
            '  <input type="text" name="actualBifocal" class="col-md-4 form-control" id="id_actualOS">' +
            ' </div>' +

            '<div class="col-md-4">' +
            '  <label class="form-label" for="email">Actual Progressive</label>' +
            '  <input type="text" name="actualProgressive" class="col-md-4 form-control" id="id_actualOD">' +
            '</div>' +

            '<div class="col-md-6">' +
            '  <label class="form-label" for="email">MediaPipe Bifocal</label>' +
            '  <input type="text" name="mediapipeBifocal" value="'+ roundToNearest5(shbmm * 100) / 100 +'" class="col-md-3 form-control" id="id_dlibOS">' +
            '</div>' +

            ' <div class="col-md-6">' +
            '   <label class="form-label" for="email">MediaPipe Progressive</label>' +
            '   <input type="text" name="mediapipeProgressive" value="'+ roundToNearest5(shmm * 100) / 100 +'" class="col-md-3 form-control" id="id_dlibOD">' +
            ' </div>' +
